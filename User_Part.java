...
public class User_Part {
	...
	public void create_user_menu(Course []courses, Student []student, Faculty []faculty, GeneralStaff []generalstaff)
	{ 
		String content="";
		int Course_c = 0;
		int Faculty_c = 0;
		...
		System.out.println("Lets create objects for Faculty/Student/Course/GeneralStaff: ");
		...
		while(!(s = scan.nextLine()).equals("q") && s.equals("c")) {
			
		    System.out.println("Which option you want to select-\n"+"1.Add 2 new Courses to a Faculty object\n" + 
		    		"2.Add 2 new Courses to a Student object\n" + 
		    		"3.Add an array of 2 Courses to a Faculty object\n" + 
		    		"4.Add an array of 2 Courses to a Student object\n"+
		    		"5.Get the Course at index (valid and invalid indexes) from a Faculty object\n" + 
		    		"6.Get the Course at index (valid and invalid indexes) from a Student object\n" + 
		    		"7.Select a Faculty object and a Course object from menus and query the Faculty object for the Course to determine whether the Faculty object teaches it or not\n"+
		    		"8.Determine which Faculty object teaches the most and the least courses\n"+
		    		"9.Determine which Course is the minimum and which is the maximum of all Course objects in the catalog\n" + 
		    		"10.Determine which Student has the most and least credits\r\n" 
		    		);
		   ...
		   if(user == 1) {
			   System.out.println("------------------------------------------------------------------------------------------------------------");
	            ...
	            content +=  "***********************************************************************************"+"\n";
	            System.out.println("\nYou want to interact with the menu or close it?\nPress 'q' to close it.\nPress 'c' to continue. ");
		   }
		   if(user == 2) {
			   System.out.println("----------------------------------------------------------------------------------------------------------");
		   ...
		   if(user == 3) {
			   System.out.println("----------------------------------------------------------------------------------------------------------");
	            ...
	            System.out.println("Two Courses: "+"\n"+"1."+fac_co1.toString()+"\n"+"2."+fac_co2.toString()+"\n");
		   ...
		   if(user == 4) {
			   System.out.println("----------------------------------------------------------------------------------------------------------");
	            ...
	            Course s_co1 = new Course(true,535,"PHY",3);
	            Course s_co2 = new Course(false,655,"DB",3);
	            System.out.println("Two Courses: "+"\n"+"1."+s_co1.toString()+"\n"+"2."+s_co2.toString()+"\n");
	            Course [] scourses = {s_co1, s_co2}; 
	            
	            
	            sr1.addCoursesTaken(scourses);
		   ...
		   if(user == 5) {
			   System.out.println("----------------------------------------------------------------------------------------------------------");
			   System.out.println("All (7) Faculty info: ");
			   Scanner ch_co = new Scanner(System.in);
				for(int k = 0 ; k <faculty.length ;k++)
				{
					System.out.println("Faculty"+(k+1)+":\n"+faculty[k].toString());
				}
				
				System.out.println("Choose a faculty to check? ");
			   ...
			   System.out.println("Check if faculty "+fac1.getName()+ " takes the course at the index?"+"\n"+"Enter index:");
	            int ind = ch_co.nextInt();
	            System.out.println("Course at index "+ind+" taken by the faculty "+fac1.getName()+" : "+fac1.getCourseTaught(ind));
	            content += "Course at index "+ind+" taken by faculty "+fac1.getName()+" : "+fac1.getCourseTaught(ind) +"\n";
	            content +=  "***********************************************************************************"+"\n";
	            System.out.println("\nYou want to interact with the menu or close it?\nPress 'q' to close it.\nPress 'c' to continue. ");
		   ...
		   if(user == 6)
		   {
			   Scanner sr_r = new Scanner(System.in);
			   System.out.println("Student (7) info: ");
	            for(int s1=0; s1<student.length; s1++)
	            {
	            	System.out.println("Student"+(s1+1)+":\n"+student[s1].toString());
	            ...
	            System.out.println("Choose a Student to check?");
	            Scanner wh_s = new Scanner(System.in);
	            int std_i = wh_s.nextInt();
	            Student s_1;
	            s_1 = student[std_i-1];
			   
			    System.out.println("Check if student "+s_1.getName()+ " takes the course at the index?"+"\n"+"Enter index:");
	            int s_ind = sr_r.nextInt();
	            System.out.println("Course at index "+s_ind+" taken by the student "+s_1.getName()+" : "+s_1.getCourseTaken(s_ind));
	            content += "Course at index "+s_ind+" taken by the student "+s_1.getName()+" : "+s_1.getCourseTaken(s_ind) +"\n";
	            content +=  "***********************************************************************************"+"\n";
		   ...
		   if(user == 7) {
			   ...
			   System.out.println("All the Faculty & Course info: ");
	            ...
	            for(int j1=0 ; j1<faculty[user_fac-1].getNumCoursesTaught();  j1++ ) {
	            	...
	            	{    teach = true;
	            		System.out.println("The faculty "+faculty[user_fac-1].getName()+" teaches course "+courses[user_co-1].getCourseName());
	            	    content += "The faculty "+faculty[user_fac-1].getName()+" teaches course "+courses[user_co-1].getCourseName() +"\n";
	            	    content += "***********************************************************************************"+"\n";
	           ...
	           if(!teach) {
	           System.out.println("The faculty "+faculty[user_fac-1].getName()+" does not teach course "+courses[user_co-1].getCourseName());
	           content += "The faculty "+faculty[user_fac-1].getName()+" does not teach course "+courses[user_co-1].getCourseName() +"\n";
	           content += "***********************************************************************************"+"\n";
		   ...
		   if(user == 8) {
			   System.out.println("----------------------------------------------------------------------------------------------------------");
	           Faculty f_most = faculty[0];
	           Faculty f_least= faculty[0];
	           int max =  faculty[0].getNumCoursesTaught();
	           int min =  faculty[0].getNumCoursesTaught();
	           ...
	           System.out.println(f_most.getName()+" (employee id "+f_most.getEmployeeID()+")"+" teaches the most courses - "+max);
	           content += f_most.getName()+" (employee id "+f_most.getEmployeeID()+")"+" teaches the most courses - "+max +"\n";
	           System.out.println(f_least.getName()+"             "+"(employee id "+f_least.getEmployeeID()+")"+" teaches the least courses - "+min);
	           content += f_least.getName()+"             "+"(employee id "+f_least.getEmployeeID()+")"+" teaches the least courses - "+min +"\n";
	           content +=  "***********************************************************************************"+"\n";
		       System.out.println("----------------------------------------------------------------------------------------------------------");
	           System.out.println("\nYou want to interact with the menu or close it?\nPress 'q' to close it.\nPress 'c' to continue. ");
